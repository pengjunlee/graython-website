<template>
  ...
  >
    <div ref="terminalRef" class="gray-terminal" :style="mainStyle">
      <a-collapse
...
const props = withDefaults(defineProps<GrayTerminalProps>(), {
  height: "400px",
  fullScreen: false,
...
onMounted(() => {
  ...
  if (welcomeTexts?.length > 0) {
    ...
    terminal.writeTextOutput(
      `^_^ Welcome to my personal terminal workstation, ` +
...
function handleClickWrapper(event: Event): void {
  //@ts-ignore
  if (event.target.className === "gray-terminal") {
    focusInput();
  }
}
...
.gray-terminal::-webkit-scrollbar {
  display: none;
}
...
.gray-terminal :deep(.ant-collapse) {
  background: none;
}

.gray-terminal :deep(.ant-collapse-borderless > .ant-collapse-item) {
  border: none;
}

.gray-terminal :deep(.ant-collapse-content > .ant-collapse-content-box) {
  padding: 0;
}
